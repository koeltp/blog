...
客户端凭证的处理逻辑在令牌端点中（已在[第二篇](/docs/openiddict/02授权码流程详解.html)中展示）。这里单独看核心部分：

```csharp title="令牌端点 - 客户端凭证分支"
...
```csharp title="资源服务器 Program.cs（OIDC 方式）"
...
```csharp title="使用 OpenIddict 验证（同项目）"
...
> **性能取舍**：引用令牌可以即时撤销，但每次 API 调用都需要网络请求。JWT 无网络开销，但无法即时撤销。详见[第四篇：令牌管理](/docs/openiddict/04令牌管理.html)。
...
> **下一篇**：[令牌管理](/docs/openiddict/04令牌管理.html) —— 刷新令牌机制、令牌撤销、引用令牌、令牌加密、安全最佳实践。