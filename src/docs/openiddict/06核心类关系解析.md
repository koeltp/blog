...
```csharp title="Descriptor 和 Entity 的区别"
...
你可以继承这些实体来添加自定义字段（详见[第五篇](/docs/openiddict/05自定义扩展与踩坑.html)）：
