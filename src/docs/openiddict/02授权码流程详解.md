...
```csharp title="AuthorizationController.cs"
...
```csharp title="TokenController.cs"
...
```csharp title="UserInfoController.cs"
...
> **下一篇**：[客户端凭证与资源服务器](/docs/openiddict/03客户端凭证与资源服务器.html) —— 服务间通信的客户端凭证流程，以及资源服务器如何验证 Access Token。