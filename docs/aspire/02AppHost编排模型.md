...
stateDiagram-v2
    ...
    note right of WaitFor: WaitFor() 等到这里
    note right of WaitForStart: WaitForStart() 等到这里
```
...
> **下一篇**：[服务通信与发现](tutorial.html?type=aspire&file=03服务通信与发现.md) —— 深入理解 Aspire 的服务发现机制、HTTP/gRPC 通信、命名端点和健康检查自动注册。
