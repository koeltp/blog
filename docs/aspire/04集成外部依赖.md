...
stateDiagram-v2
    ...
    note right of Pulling: docker pull redis:latest
    note right of Creating: docker create --name cache ...
    note right of Starting: docker start cache
```
...
> **下一篇**：[可观测性与 Dashboard](tutorial.html?type=aspire&file=05可观测性与Dashboard.md) —— 深入理解 Aspire 的 OpenTelemetry 自动配置、日志/追踪/指标三支柱、Dashboard 深入使用、自定义遥测。
