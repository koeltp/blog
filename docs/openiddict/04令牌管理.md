...
资源服务器使用**内省端点**验证引用令牌（详见[第三篇](tutorial.html?type=openiddict&file=03客户端凭证与资源服务器.md)方式三）：
...
```csharp name="检测 Refresh Token 重放"
// 在令牌端点的刷新流程中
...
> **下一篇**：[自定义扩展与踩坑](tutorial.html?type=openiddict&file=05自定义扩展与踩坑.md) —— 自定义实体和 Store、与 ASP.NET Core Identity 集成、常见踩坑排查。